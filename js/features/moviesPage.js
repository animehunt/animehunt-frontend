// ============================================================
// js/features/moviesPage.js
// movies.html — movies listing with pagination
// HTML mein: <body data-page="movies">
// Grid: <div class="anime-grid" data-type-page="movie">
// ============================================================
...
async function loadPage(page, replace = false) {
  ...
  try {
    const data = await fetchListing('movie', page, 'all');
...
function renderGrid(items, replace = false) {
  const grid = document.querySelector('.anime-grid[data-type-page="movie"]');
  if (!grid) return;
...
function renderPagination(page, total) {
  ...
  let html = `<button ${page === 1 ? 'disabled' : ''}
    onclick="window.__moviesPage(${page - 1})">Prev</button>`;

  for (let p = start; p <= end; p++) {
    html += `<button class="${p === page ? 'active' : ''}"
      onclick="window.__moviesPage(${p})">${p}</button>`;
  }

  html += `<button ${page === totalPages ? 'disabled' : ''}
    onclick="window.__moviesPage(${page + 1})">Next</button>`;

  nav.innerHTML = html;
  window.scrollTo({ top: 0, behavior: 'smooth' });
}

window.__moviesPage = async (page) => {
...
function initAZNav() {
  document.querySelectorAll('.az-nav span').forEach(span => {
    span.addEventListener('click', () => {
      ...
      window.location.href = `Category.html?letter=${letter}&type=movie`;
    });
  });
}
