...
// HTML mein: <body data-page="cartoon">
// Grid: <div class="cartoon-grid" data-type-page="cartoon">
// ============================================================
...
function renderPagination(page, total) {
  ...
  let html = `<button ${page === 1 ? 'disabled' : ''}
    onclick="window.__cartoonPage(${page - 1})">Prev</button>`;

  for (let p = start; p <= end; p++) {
    html += `<button class="${p === page ? 'active' : ''}"
      onclick="window.__cartoonPage(${p})">${p}</button>`;
  }

  html += `<button ${page === totalPages ? 'disabled' : ''}
    onclick="window.__cartoonPage(${page + 1})">Next</button>`;

  nav.innerHTML = html;
  window.scrollTo({ top: 0, behavior: 'smooth' });
}

window.__cartoonPage = async (page) => {
